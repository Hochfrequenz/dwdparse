...
def test_mosmix_parser_skips_station_without_coordinates(caplog):
    """The skip used to depend on catching the AttributeError raised by
    the missing element, so it has to keep working without it."""
    place = _placemark(
...
def test_mosmix_parser_rejects_station_without_name():
    ...
    which did not say which element was missing."""
...
def test_mosmix_parser_rejects_station_without_description():
    """Same failure as a missing name, from a different element."""
...
def test_mosmix_parser_allows_station_with_empty_description():
    """An empty description has always yielded a record with no station
    name, and must not start raising."""
    place = _placemark(
        '<kml:name>01049</kml:name>'
        '<kml:description/>'
